    ...
    <section className="cursor-pointer select-none relative w-full max-w-xs mx-auto bg-white rounded-xl overflow-hidden shadow-xl group hover:shadow-2xl transition-all duration-300 h-96 sm:h-[28rem]">
      ...
      <div className="relative z-10 p-4 sm:p-6 h-full flex flex-col justify-between">
        ...
        <div className="text-center pt-2 sm:pt-4">
          <h3 className="text-white text-lg sm:text-xl font-bold leading-tight px-2 break-words">
            {title}
          </h3>
        </div>
        {/* Icon Bar - Bottom with 3 items */}
        <div className="flex flex-col sm:flex-row justify-center gap-2 sm:gap-3 pb-2">
    {activities.slice(0, 3).map((activity, index) => (
        <div
            key={index}
            // 👇 Alterações nesta linha 👇
            className="flex flex-col items-center justify-center gap-1 bg-white/20 backdrop-blur-sm rounded-lg px-3 py-2 text-white min-w-0 w-full sm:flex-1"
        >
